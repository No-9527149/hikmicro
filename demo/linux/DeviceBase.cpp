...
void DeviceBase::FuncLoginDevices(int iDevIndex) {
  ...
  int lUserID = USB_Login(&m_struCurUsbLoginInfo, &struDeviceRegRes);
...
void DeviceBase::SetSystemUpdateFirmware() {
  ...
  if (m_nTimeout < 15 || m_nTimeout > 255) {
    printf(
        "Timeout time range should between 15(include) and 255(include)\r\n");
    return;
  }
  ...
  while (m_bUpgrade) {
    ...
    if (!USB_GetUpgradeState(m_lUpgradeHandle, &m_struUpgradeState)) {
      ...
      printf("m_lUpgradeHandle = [%ld] byState = [%d], byProgress = [%d]\r\n",
             m_lUpgradeHandle, m_struUpgradeState.byState,
             m_struUpgradeState.byProgress);
      ...
      switch (m_struUpgradeState.byState) {
        ...
        case USB_UPGRADE_SUCCESS: {
          ...
          printf(
              "Before operate this device,please enum and login device "
              "again\r\n");
          break;
        }
