...
void CGetStream::DeviceCallBackLocal(LONG handle, USB_FRAME_INFO *pFrameInfo,
                                     void *pUser) {
  printf(
      "DeviceCallBackLocal:handle[%d], StreamType[%d], FrameLen[%d], "
      "RealFrameRate[%d]\n",
      handle, pFrameInfo->dwStreamType, pFrameInfo->dwBufSize,
      pFrameInfo->dwFrameRate);
  return;
...
void CGetStream::StartStreamRecord() {
  ...
  if (!USB_StopChannel(m_lUserID, m_iRecordChannel)) {
    ...
    printf("success USB_StopChannel\r\n");
  }
}
...
BOOL CGetStream::SetVideoParam() {
  ...
  if (USB_SetDeviceConfig(m_lUserID, USB_SET_VIDEO_PARAM, &struConfigInputInfo2,
                          &struConfigOutputInfo2)) {
